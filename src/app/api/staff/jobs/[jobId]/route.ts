...
export async function GET(_request: Request, { params }: { params: { jobId: string } }) {
  ...
  return NextResponse.json({ job: { ...job, attemptsLog: job.attemptsLog.map(attempt => ({ ...attempt, result: storedAttemptResult(attempt.stats, { id: job.id, type: job.type, status: attempt.status }) })) } })
}
