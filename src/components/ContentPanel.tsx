...
import { choicesFromRows } from './CategoryTreeControl'
...
function categoryPath(rows: {id:string;parentId:string|null;name:string}[], id: string | null) {
  if (!id) return ''
  const byId = new Map(rows.map(row => [row.id, row]))
  const names: string[] = [], seen = new Set<string>()
  let current = byId.get(id)
  while (current && !seen.has(current.id)) { seen.add(current.id); names.unshift(current.name); current = current.parentId ? byId.get(current.parentId) : undefined }
  return names.join(' / ')
}
export function ContentPanel() {
  ...
  const selectedCategoryPath = useMemo(() => categoryPath(categories.data ?? [], draft?.categoryId ?? null), [categories.data, draft?.categoryId])
  ...
  const mutate = async (url: string, method: string, data?: unknown) => {
    ...
    try {
      ...
      setCategoryDialogOpen(false); setDraft(null); setRemoving(null); setMessage(method === 'DELETE' ? 'Баннер удалён.' : 'Баннер сохранён.'); await banners.reload()
  ...
  return <div>
    ...
    <button className="button button-primary" disabled={busy} onClick={() => { setCategoryDialogOpen(false); setDraft(empty()); setMessage('') }}>Добавить баннер</button>
    ...
    {draft ? <form className="admin-banner-form" onSubmit={e => { e.preventDefault(); const { id, ...data } = draft; void mutate('/api/staff/banners' + (id ? '/' + id : ''), id ? 'PATCH' : 'POST', data) }}>
      ...
      <fieldset disabled={busy} style={{ border: 0, padding: 0, minWidth: 0 }}>
        ...
        <div className="admin-toolbar"><label>Где показывать<select value={draft.placement} onChange={e => { setCategoryDialogOpen(false); patch({ placement: e.target.value as Banner['placement'], categoryId: null, brandId: null }) }}><option value="CATALOG">Каталог</option><option value="HOME">Главная</option></select></label>
        </div>
        {draft.placement==='CATALOG'?<fieldset className="banner-category-picker"><legend>Категория показа</legend>
          <div className="banner-category-selection"><span><small>Выбрано</small><strong>{selectedCategoryPath || (draft.brandId ? 'Прежняя привязка к бренду' : 'Весь каталог')}</strong></span><div><button type="button" className="button button-secondary" disabled={categories.loading || Boolean(categories.error) || tree.length === 0} onClick={()=>setCategoryDialogOpen(true)}>Выбрать категорию</button>{draft.categoryId || draft.brandId ? <button type="button" onClick={()=>patch({categoryId:null,brandId:null})}>Весь каталог</button> : null}</div></div>
          <p>{draft.categoryId?'Баннер будет показан в выбранной категории и во всех вложенных папках.':draft.brandId?'Сохранена прежняя привязка к бренду. Выберите категорию, чтобы изменить место показа.':'Баннер будет показан во всём каталоге.'}</p>
          {categories.loading?<p role="status">Загрузка дерева…</p>:categories.error?<p role="alert">{categories.error} <button type="button" onClick={categories.reload}>Повторить</button></p>:!tree.length?<p>В каталоге пока нет категорий.</p>:null}
          <CategoryTreeDialog open={categoryDialogOpen} nodes={tree} selectedId={draft.categoryId} onClose={()=>setCategoryDialogOpen(false)} onSelect={node=>patch({categoryId:node.id,brandId:null})}/>
        </fieldset>:null}
        ...
        <div className="admin-toolbar"><button type="submit" className="button button-primary" disabled={draft.isActive && !draft.desktopImageUrl}>Сохранить баннер</button><button type="button" className="button button-secondary" onClick={() => { setCategoryDialogOpen(false); setDraft(null) }}>Отмена</button></div>
      </fieldset>
    ...
    {banners.data?.map(b => <article className="admin-banner-row" key={b.id}>
      ...
      <div className="admin-banner-actions"><button disabled={busy} onClick={() => { setCategoryDialogOpen(false); setDraft(b); setMessage('') }}>Редактировать</button><button disabled={busy} onClick={() => mutate('/api/staff/banners/' + b.id, 'PATCH', { isActive: !b.isActive })}>{b.isActive ? 'Выключить' : 'Включить'}</button><button disabled={busy} onClick={() => setRemoving(b.id)}>Удалить</button></div>
