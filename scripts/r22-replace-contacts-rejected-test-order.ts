/** Only isolated R22-00002, rejected during warehouse extraction per user trace. Preserve all prior XML/attempt history. */
...
async function main(){
 ...
 const plan=JSON.parse(fs.readFileSync('/app/r22-config/test-order-plan.json','utf8'));const proof=JSON.parse(fs.readFileSync('/app/r22-config/contacts-error-proof.json','utf8'));
 if(proof.source!=='User-provided UT 11.4 stack trace in this conversation'||proof.reportedError!=='Поле объекта не обнаружено (Контакты)'||proof.line!==4657||proof.orderNumber!=='R22-00002')throw Error('rejection_evidence_required');
 ...
 const old=await db.order.findUniqueOrThrow({where:{id:'cmubm9m0y000fthi0z0l9d28b'},include:{export:true,user:true}});
 if(old.storeId!==store.id||old.number!=='R22-00002'||old.export?.externalId||old.export?.submittedAt||old.export?.confirmedAt||await db.invoice.count({where:{orderId:old.id}}))throw Error('old_order_not_proven_unconfirmed');
 if(await db.checkoutReceipt.findUnique({where:{key:'r22-warehouse-contacts-retry'}}))throw Error('replacement_already_exists_inspect_it');
 if(old.status!=='SUBMITTED'||old.export?.status!=='AWAITING_ACK'||old.export.attempts!==1)throw Error('old_order_state_changed');
 const before=await db.onecSaleDelivery.findUniqueOrThrow({where:{exportId:old.export.id}});
 ...
 const next=await checkout(old.user,{deliveryLocationId:old.deliveryLocationId,comment:plan.comment+'; повтор R22-00002 после ошибки чтения контактов склада',idempotencyKey:'r22-warehouse-contacts-retry'});
 ...
 await db.$transaction(async tx=>{
  ...
  const affected=await tx.orderExport.updateMany({where:{id:old.export!.id,status:'AWAITING_ACK',submittedAt:null,externalId:null},data:{status:'FAILED',lastError:'UT114 warehouse contacts parsing failed per user trace; no ACK received; replaced by '+next.number}});
  if(affected.count!==1)throw Error('old_export_changed');
  await recordAudit(tx,{storeId:store.id,actor,action:'OnecTestOrderReplacedAfterReportedRejection',targetType:'Order',targetId:old.id,metadata:{replacementOrderId:next.id,userTraceSha256:proof.sourceSha256}});
 });
 ...
 if(previous.sha256!==before.sha256||previous.xml!==before.xml||(await db.orderExport.findUniqueOrThrow({where:{id:old.export.id}})).attempts!==old.export.attempts)throw Error('historical_delivery_changed');
 ...
 fs.writeFileSync('/tmp/r22-contacts-replacement.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report));
}
