...
export type StorefrontBanner = { id: string; name: string; desktopImageUrl: string | null; mobileImageUrl: string | null; linkUrl: string | null; brand: { slug: string; name: string } | null }
export function StoreBanners({ banners }: { banners: StorefrontBanner[] }) {
  return <div className="store-banners" aria-label="Акции магазина">{banners.map(banner => {
    const href = safeBannerHref(banner.linkUrl) || (banner.brand ? '/brands/' + encodeURIComponent(banner.brand.slug) : null)
